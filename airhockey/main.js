...
let gameOverTime = 0;
// オブジェクト初期化
let puck = { x: 200, y: 300, vx: 0, vy: 0, radius: 15, touchId: null };
let p1 = { x: 200, y: 500, vx: 0, vy: 0, radius: 25, touchId: null };
let p2 = { x: 200, y: 100, vx: 0, vy: 0, radius: 25, touchId: null };
// --- 物理計算ユーティリティ ---
function getClosestPointOnSegment(A, B, C) {
    const abX = B.x - A.x;
    const abY = B.y - A.y;
...
function updatePhysics() {
    ...
    const speed = Math.sqrt(Math.pow(puck.vx, 2) + Math.pow(puck.vy, 2));
    if (speed > MAX_SPEED) {
...
function handleTouch(e) {
    ...
    Array.from(e.touches).forEach(t => {
        ...
        if (p1.touchId === null && ty > mid)
            p1.touchId = t.identifier;
        if (p2.touchId === null && ty < mid)
            p2.touchId = t.identifier;
...
canvas.addEventListener("touchend", (e) => {
    ...
    Array.from(e.changedTouches).forEach(t => {
        if (p1.touchId === t.identifier) {
            p1.touchId = null;
            p1.vx = 0;
            p1.vy = 0;
        }
        if (p2.touchId === t.identifier) {
            p2.touchId = null;
            p2.vx = 0;
            p2.vy = 0;
        }
...
canvas.addEventListener("mousedown", () => { if (isGameOver)
    restartGame(); });
// --- 描画 ---
function draw() {
